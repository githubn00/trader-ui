...
class Di2 extends ce{
  ...
  _drawGraph(t){const{data:s,settings:e}=this,{style:i,params:p2}=e;(s&&this.drawHistogram(t,s,i.line),p2.showBidZero&&i.bidZero&&i.bidZero.visible&&this._bidZeroDiffD&&this.drawLine(t,this._bidZeroDiffD,i.bidZero));if(p2.showBidZero&&p2.showBidZeroLabel&&i.bidZero&&this._bidZeroDiffD){const bars=this.chart.bars,d=this._bidZeroDiffD,st=this.chart.state,step=st.getStep(),from=st.getFrom(),count=st.getCount(),digits=this.chart.digits,col=i.bidZero.color;const fmtD=(v)=>(v>=0?'+':'')+v.toFixed(digits);for(const cr of(this._bidZeroCrossesD||[])){if(cr.idx<from||cr.idx>from+count+1)continue;const cx=st.startX()+(cr.idx-from)*step,cy=this.valueToY(cr.prevVal),tx=this.createText(cr.price.toFixed(digits));if(tx){tx.tint=col;tx.x=cx+4;tx.y=cy-8;t.addChild(tx);}const ts=this.createText(fmtD(cr.prevVal));if(ts){ts.tint=col;ts.x=cx+4;ts.y=cy+4;t.addChild(ts);}}const lastIdx=d.length-1,tipX=st.startX()+(lastIdx-from)*step,tipY=this.valueToY(d[lastIdx]),tx2=this.createText(bars.close(lastIdx).toFixed(digits));if(tx2){tx2.tint=col;tx2.x=tipX+4;tx2.y=tipY-8;t.addChild(tx2);}const ts2=this.createText(fmtD(d[lastIdx]));if(ts2){ts2.tint=col;ts2.x=tipX+4;ts2.y=tipY+4;t.addChild(ts2);}}}
}
