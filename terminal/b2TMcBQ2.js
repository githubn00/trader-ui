...
function Ni2(b){const m={512:1/6,1:1,2:5,4:15,8:30,16:60,32:240,64:1440,128:10080,256:43200};return m[b]||0;}
...
class Wi2 extends ce{
  ...
  _calc(t){
    ...
    if(s.showBidZero){
      ...
      for(let j=1;j<len;j++){if((i[j-1]<0&&i[j]>=0)||(i[j-1]>0&&i[j]<=0)){const prevVal=bidDiff[j-1];bidAtCross=bars.close(j);crosses.push({idx:j,price:bidAtCross,prevVal:prevVal,dir:i[j-1]<0?1:-1});}bidDiff[j]=isNaN(bidAtCross)?0:bars.close(j)-bidAtCross;}
  ...
  _drawGraph(t){const{data:s,settings:e}=this,{style:i,params:p2}=e;(i.line.visible&&s&&this.drawHistogram(t,s,i.line),i.signal.visible&&vs(this,it2)&&this.drawLine(t,vs(this,it2),i.signal),p2.showBidZero&&i.bidZero&&i.bidZero.visible&&this._bidZeroDiffW&&this.drawLine(t,this._bidZeroDiffW,i.bidZero));if(p2.showBidZero&&p2.showBidZeroLabel&&i.bidZero&&this._bidZeroDiffW){const bars=this.chart.bars,d=this._bidZeroDiffW,st=this.chart.state,step=st.getStep(),from=st.getFrom(),count=st.getCount(),digits=this.chart.digits;const fmtD=(v)=>(v>=0?'+':'')+v.toFixed(digits),diffCol=(v)=>v>=0?0x00ff44:0xff4444;for(const cr of(this._bidZeroCrossesW||[])){if(cr.idx<from||cr.idx>from+count+1)continue;const cx=st.startX()+(cr.idx-from)*step,cy=this.valueToY(cr.prevVal),tx=this.createText(cr.price.toFixed(digits));if(tx){tx.tint=cr.dir>0?0xff8844:0x44aaff;tx.x=cx+4;tx.y=cy-8;t.addChild(tx);}const ts=this.createTextSmall(fmtD(cr.prevVal));if(ts){ts.tint=diffCol(cr.prevVal);ts.x=cx+4;ts.y=cy+6;(this.labelContainer||t).addChild(ts);}}const lastIdx=d.length-1;if(lastIdx<0)return;const tipX=st.startX()+(lastIdx-from)*step,tipY=this.valueToY(d[lastIdx]),tx2=this.createText(bars.close(lastIdx).toFixed(digits));if(tx2){tx2.tint=s[lastIdx]>=0?0x44aaff:0xff8844;tx2.x=tipX+4;tx2.y=tipY-8;t.addChild(tx2);}const ts2=this.createTextSmall(fmtD(d[lastIdx]));if(ts2){ts2.tint=diffCol(d[lastIdx]);ts2.x=tipX+4;ts2.y=tipY+6;(this.labelContainer||t).addChild(ts2);}}}
}
...
class Di2 extends ce{
  ...
  _drawGraph(t){const{data:s,settings:e}=this,{style:i,params:p2}=e;(s&&this.drawHistogram(t,s,i.line),p2.showBidZero&&i.bidZero&&i.bidZero.visible&&this._bidZeroDiffD&&this.drawLine(t,this._bidZeroDiffD,i.bidZero));if(p2.showBidZero&&p2.showBidZeroLabel&&i.bidZero&&this._bidZeroDiffD){const bars=this.chart.bars,d=this._bidZeroDiffD,st=this.chart.state,step=st.getStep(),from=st.getFrom(),count=st.getCount(),digits=this.chart.digits;const fmtD=(v)=>(v>=0?'+':'')+v.toFixed(digits),diffCol=(v)=>v>=0?0x00ff44:0xff4444;for(const cr of(this._bidZeroCrossesD||[])){if(cr.idx<from||cr.idx>from+count+1)continue;const cx=st.startX()+(cr.idx-from)*step,cy=this.valueToY(cr.prevVal),tx=this.createText(cr.price.toFixed(digits));if(tx){tx.tint=cr.dir>0?0xff8844:0x44aaff;tx.x=cx+4;tx.y=cy-8;t.addChild(tx);}const ts=this.createTextSmall(fmtD(cr.prevVal));if(ts){ts.tint=diffCol(cr.prevVal);ts.x=cx+4;ts.y=cy+6;(this.labelContainer||t).addChild(ts);}}const lastIdx=d.length-1;if(lastIdx<0)return;const tipX=st.startX()+(lastIdx-from)*step,tipY=this.valueToY(d[lastIdx]),tx2=this.createText(bars.close(lastIdx).toFixed(digits));if(tx2){tx2.tint=s[lastIdx]>=0?0x44aaff:0xff8844;tx2.x=tipX+4;tx2.y=tipY-8;t.addChild(tx2);}const ts2=this.createTextSmall(fmtD(d[lastIdx]));if(ts2){ts2.tint=diffCol(d[lastIdx]);ts2.x=tipX+4;ts2.y=tipY+6;(this.labelContainer||t).addChild(ts2);}}}
}
